...
module.exports = {
    ...
    bot: async (urlToVisit) => {
        ...
        try {
            ...
            await page.goto(CONFIG.APPURL+"/wp-login.php")

            const user_login = await page.$("#user_login")
            const user_pass = await page.$("#user_pass")
            const submit = await page.$("#wp-submit")

            await user_login.type(username.toString())
            await user_pass.type(password.toString())
            await submit.click()
